...
def test_examples_stack_files_load_individually() -> None:
    """验证 tests/fixtures/example-project/stacks 下每个示例 stack 文件都能独立加载。"""
    stack_paths = sorted(Path("tests/fixtures/example-project/stacks").glob("*.yaml"))
...
def test_stack_rejects_missing_published_socks_password() -> None:
    ...
    stack_data = load_yaml(Path("tests/fixtures/example-project/stacks/usa1.yaml"))
