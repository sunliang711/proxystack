...
- `list [stack]`：以树形结构列出全部 stack 或指定 stack 的 enabled 状态、整体运行状态、组件生成状态和主要监听端口；端口后缀 `(L)` 表示 localhost/回环地址，`(*)` 表示非回环地址；`--verbose` 可展开 Xray API、mihomo controller 和完整 `host:port`；默认不做系统端口占用检查，需要严格检查时使用 `--check-system-ports`。
...
managed_config:
  enabled: true
  public_base_url: https://www.rustez.cc/api/sub
  interval: 86400
  strict: true
```
