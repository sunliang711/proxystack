...
1. 建立测试目录结构：`tests/unit`、`tests/golden`、`tests/fixtures`、`tests/e2e`。
2. 覆盖 `examples/config.yaml` 和 `examples/stacks/*.yaml` 的加载与校验。
3. 覆盖非法配置：端口冲突、ref 缺失、循环依赖、危险 socks/http 暴露、必填凭据缺失、重复订阅 node id。
4. 覆盖 `add/clone --allocate-ports` 的端口池分配和写回行为。
5. 覆盖 Xray、mihomo、subscription 生成器 golden 输出。
6. 覆盖 `plan/apply/up` 的职责边界：`plan` 不写文件，`apply` 不操作服务，`up` 才启动或重启变化服务。
7. 覆盖订阅发布包 import 默认 rebuild、`--no-rebuild` 跳过 rebuild、current 原子切换。
8. 覆盖 agent/sub 同机部署目录边界和锁隔离。
9. 覆盖订阅 HTTP token 鉴权、无用户、空节点和三类订阅格式。
10. 覆盖 Docker Compose 示例的关键安全配置：非 root、只读 rootfs、`cap_drop: ALL`、持久化 `/data`、healthcheck。
