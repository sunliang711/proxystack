...
7. [x] 覆盖订阅发布包 import 默认增量写入、`--replace-all` 全量替换、inputs 原子替换和 watcher reload。
...
- 端到端测试覆盖 `init -> add -> validate -> check -> start -> sub export -> sub import -> serve` 的主流程。
...
- 已补充 Task11 增量测试，覆盖示例 stack 独立加载、必填凭据缺失、`add` 默认端口写回、订阅 input/index/格式 golden、`proxystack-sub import` 增量写入、`--replace-all` 全量替换、watcher reload、fake `serve` 成功路径、Docker Compose 安全配置、agent/sub 目录边界和锁路径文档约束。
- 已补充端到端主流程测试：`init -> add -> validate -> check -> start -> sub export -> sub import -> serve`，通过 fake systemd runner 和 fake uvicorn 隔离真实系统服务和网络监听。
