...
1. 编写 `scripts/lib/common.sh`，包含 `log`、`warn`、`die`、`run`、`require_cmd`、`require_root`、`guard_managed_path`、`ensure_dir`、`ensure_venv` 等函数。
2. 编写 `scripts/install-agent.sh`，支持 `--wheel`、`--source`、`--package`、`--base-dir`、`--bin-dir`、`--python`、`--user`、`--group`、`--no-init`、`--install-systemd`、`--dry-run`。
3. 编写 `scripts/install-sub-local.sh`，支持 `--wheel`、`--source`、`--package`、`--base-dir`、`--import-bundle`、`--install-systemd`、`--start`、`--dry-run`。
4. 编写 `scripts/deploy-sub-docker.sh`，支持 `--image`、`--name`、`--data-dir`、`--host`、`--port`、`--user`、`--data-owner`、`--pull`、`--replace`、`--dry-run`。
5. 编写 `scripts/README.md`，说明三类脚本的使用方式、安全边界和常见示例。
6. 在部署文档中引用脚本任务和最终脚本入口，不把脚本职责扩展到后续更新。
