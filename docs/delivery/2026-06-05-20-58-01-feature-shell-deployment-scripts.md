...
- 新增 `scripts/install-sub-local.sh`，支持本地非 Docker 订阅服务安装、sub 数据目录创建、可选发布包导入、可选安装和启动 `ps-sub.service`。
...
- `install-sub-local.sh` 为了安装 `ps-sub.service` 需要可读取的 `config.yaml`；当配置不存在时会生成默认配置，但不会覆盖已有配置。
