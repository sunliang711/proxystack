...
- 示例 `examples/config.yaml` 默认开启 API、Stats、Policy。
...
- `/tmp/proxystack-test-venv/bin/python -m proxystack.cli.agent validate -c examples/config.yaml --skip-system-ports`
- `/tmp/proxystack-test-venv/bin/python -m proxystack.cli.agent check -c examples/config.yaml --skip-system-ports`
- `git diff --check`
